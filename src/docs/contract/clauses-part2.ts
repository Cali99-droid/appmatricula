...
export function addClausesPart2(doc: InstanceType<typeof PDFDocument>) {
  ...
  const C6_P5A = `6.5	Que, conoce que es de propiedad del Colegio toda la información y material provisto u obtenido por él, en la ejecución`;
  const C6_P5B = `del Servicio, ello incluye de modo enunciativo, mas no limitativo, material audiovisual, bibliográfico, separatas, modelos`;
  ...
  const C6_P7A = `6.7	Que, el Colegio no se responsabiliza por la suspensión de actividades por caso fortuito o fuerza mayor, o caída de los`;
  const C6_P7B = `servidores de la Plataforma virtual quedando el PADRE O MADRE DE FAMILIA comprometidas a acatar cualquier`;
  const C6_P7C = `medida extraordinaria para la recuperación de clases que el Colegio apruebe para el cumplimiento del plan de estudios.`;

  const C6_P8A = `6.8 Que, en caso de que el Alumno exhiba ”Deficiencia Académica” en dos bimestres o trimestres consecutivos, y sale`;
  const C6_P8B = `desaprobado en 4 o más áreas (secundaria) voluntariamente solicitará el traslado del plantel en el presente año`;
  ...
  const C6_P9D = `realizados con el Colegio, éste podrá condicionar la matrícula o incluso, según lo determinen los estamentos`;
  const C6_P9E = `correspondientes o la situación lo amerite, prohibir la renovación de la matrícula al año siguiente.`;
  ...
  const C6_P15A = `6.15	Reconoce que para el acceso a la plataforma virtual requerirá tener una computadora, laptop, con acceso`;
  const C6_P15B = `OBLIGATORIO DE AUDIO, VIDEO Y MICRÓFONO con las siguientes condiciones mínimas:`;
